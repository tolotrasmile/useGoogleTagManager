...
/**
 * Example usage:
 * useGoogleTagManager('GTM-1234567) or useGoogleTagManager('GTM-1234567', {'myDataLayerName', 'myAuthKey', 'myEnvironmentName'})
 */
...
const initScripts = (
  id,
  { dataLayerName = "dataLayer", auth = "", env = "" }
) => {
  /**
   * As described: https://developers.google.com/tag-manager/quickstart
   */
  const gtmHeadScript = `(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl+'&gtm_auth=${auth}&gtm_preview=${env}&gtm_cookies_win=x';f.parentNode.insertBefore(j,f);})(window,document,'script','${dataLayerName}','${id}');`;
  const gtmBodyScript = `<iframe src="https://www.googletagmanager.com/ns.html?id=${id}&gtm_auth=${auth}&gtm_preview=${env}&gtm_cookies_win=x"height="0" width="0" style="display:none;visibility:hidden"></iframe>`;
